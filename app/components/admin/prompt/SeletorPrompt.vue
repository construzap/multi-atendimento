...
import ModalEditorPrompt from '~/components/admin/prompt/ModalEditorPrompt.vue'
...
async function onSalvar(payload: { titulo: string; conteudo: string; principal: boolean }) {
  try {
...
<template>
  <div class="overflow-hidden rounded-2xl border border-outline/40 bg-surface-container-lowest shadow-sm dark:border-dark-outline/40 dark:bg-dark-surface-container-low">
    ...
    <ModalEditorPrompt
      :open="promptModalAberto"
      :salvando="promptSalvando"
      @update:open="(v) => { if (!v) adminStore.fecharModalPrompt() }"
      @save="onSalvar"
      @delete="onExcluir"
    />
  </div>
