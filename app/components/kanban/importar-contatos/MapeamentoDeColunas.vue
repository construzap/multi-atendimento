...
defineOptions({ name: 'KanbanMapeamentoDeColunas' })
...
async function garantirDadosFunil() {
  ...
  try {
    await Promise.all([
      atendentesStore.ensureListLoaded(wid),
      kanbanStore.ensureBoardLoaded(wid),
    ])
  } catch (err: unknown) {
...
async function confirmarExcluirCampo() {
  ...
  const wid = props.workspaceId
  if (!campo || !Number.isFinite(wid) || wid < 1) {
  ...
  >
    <template #subtitle="{ totalLinhasDados: total }">
      <span class="block leading-relaxed">
        Conecte as colunas do seu arquivo aos campos do kanban e do contato.
