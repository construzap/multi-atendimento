...
const qtdNotificacoesIaLabel = computed(() => {
  const n = qtdNotificacoesIaPendentes.value
  if (n < 1) return ''
...
function abrirCriarPedido(e: Event) {
  e.stopPropagation()
  if (Date.now() - lastDragEndAt < 250) return
  ...
  >
      ...
      <div class="flex min-w-0 items-start gap-3">
        ...
        <div class="min-w-0">
          <div class="flex min-w-0 items-center gap-1.5">
            <h3 class="min-w-0 truncate font-headline text-sm font-bold text-slate-900 dark:text-dark-on-surface">
              {{ titleDisplay }}
            </h3>
            <span
              v-if="canalNomeBadge"
              class="inline-flex max-w-[9rem] shrink-0 items-center gap-1 rounded-full bg-emerald-900/90 px-2 py-0.5 text-[11px] font-semibold text-emerald-50 shadow-sm dark:bg-emerald-950 dark:text-emerald-100"
    ...
    <div class="mt-4 flex items-center justify-between gap-2">
      <div class="flex min-w-0 flex-1 flex-wrap items-center gap-1.5">
        <div
          class="group/ia inline-flex shrink-0 items-center gap-1"
          @click.stop
          @mousedown.stop
        >
          <button
          ...
          <button
            type="button"
            class="inline-flex h-6 w-6 items-center justify-center rounded-full border border-violet-200 bg-violet-50 text-violet-700 transition-all hover:bg-violet-100 dark:border-violet-800/50 dark:bg-violet-950/40 dark:text-violet-200 dark:hover:bg-violet-950/70"
            :class="temNotificacoesIaPendentes
              ? 'opacity-0 pointer-events-none group-hover/ia:opacity-100 group-hover/ia:pointer-events-auto'
              : 'opacity-100'"
            aria-label="Criar pedido pronto"
    ...
    <ModalCriarPedido
      ...
      :conversa-key="card.conversa_key"
      :canal-id="card.id_canal"
      :cliente-nome="card.name ?? card.name_group"
      :cliente-telefone="card.phone"
      :canal-nome="card.canal_nome"
    />
  </article>
</template>
