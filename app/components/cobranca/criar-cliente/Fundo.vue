...
import Cliente from '~/components/cobranca/criar-cliente/cliente/Cliente.vue'
import Mensagem from '~/components/cobranca/criar-cliente/mensagem/Mensagem.vue'
import Pagamento from '~/components/cobranca/criar-cliente/pagamento/Pagamento.vue'
import Produtos from '~/components/cobranca/criar-cliente/produtos/Produtos.vue'
...
const dataInicio = ref(agoraDataLocal())
...
function validarPasso(passo: number, irParaPassoComErro = true): boolean {
  if (passo === 1) {
    if (produtos.value.length === 0) {
      toast.error('Adicione ao menos um produto.')
      if (irParaPassoComErro) passoAtual.value = 1
      return false
    }
    ...
    if (produtos.value.some((p) => !Number.isFinite(p.quantidade) || p.quantidade <= 0)) {
  ...
  if (passo === 2) {
    if (!clienteSelecionado.value.trim()) {
      ...
      if (irParaPassoComErro) passoAtual.value = 2
      return false
    }
  ...
  if (passo === 3) {
    if (!dataInicio.value) {
      ...
      if (irParaPassoComErro) passoAtual.value = 3
      return false
    }
    ...
    if (tipoCobranca.value === 'unico' && !frequenciaCobranca.value) {
      toast.error('Selecione a frequência da cobrança.')
      if (irParaPassoComErro) passoAtual.value = 3
      return false
    }
    if (tipoCobranca.value === 'parcelado' && totalParcelas.value < 2) {
      toast.error('Informe ao menos 2 parcelas.')
      if (irParaPassoComErro) passoAtual.value = 3
      return false
    }
    if (tipoCobranca.value === 'assinatura' && !frequenciaRecorrencia.value) {
      toast.error('Selecione a frequência da assinatura.')
      if (irParaPassoComErro) passoAtual.value = 3
      return false
    }
    return true
  ...
  if (passo === 4) {
    if (!templateMensagem.value.trim()) {
      ...
      if (irParaPassoComErro) passoAtual.value = 4
      return false
    }
...
function validarAtePasso(passoDestino: number): boolean {
  for (let p = 1; p < passoDestino; p += 1) {
    if (!validarPasso(p)) return false
  }
  return true
}

function avancar() {
  if (passoAtual.value >= passos.length) return
  if (!validarPasso(passoAtual.value, false)) return
  passoAtual.value += 1
}
...
function irParaPasso(destino: number) {
  if (destino === passoAtual.value) return
  if (destino < 1 || destino > passos.length) return
  if (destino < passoAtual.value) {
    passoAtual.value = destino
    return
  }
  if (!validarAtePasso(destino)) return
  passoAtual.value = destino
}
...
function resetarFormulario() {
  ...
  dataInicio.value = agoraDataLocal()
...
function carregarDaCobranca(cobranca: Cobranca) {
  ...
  dataInicio.value = (cobranca.data_inicio || '').slice(0, 10) || agoraDataLocal()
  ...
  const partes = cobranca.data_proxima_notificacao
    ...
    : null
  dataProximaLocal.value = partes?.data || agoraDataLocal()
...
function validarFormulario(): boolean {
  return validarAtePasso(passos.length + 1)
}
    ...
    <div class="border-b border-outline/20 px-4 py-6 dark:border-dark-outline/20 sm:px-8">
      <ol class="mx-auto flex max-w-4xl items-start justify-between">
        ...
        >
          ...
          <button
            type="button"
            class="relative z-10 flex h-8 w-8 items-center justify-center rounded-full font-label text-sm font-semibold transition hover:opacity-90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40"
            :class="passoAtual >= passo.id
            ...
            :aria-current="passoAtual === passo.id ? 'step' : undefined"
            :aria-label="`Ir para etapa ${passo.label}`"
            @click="irParaPasso(passo.id)"
          >
            <svg
          ...
          <button
            type="button"
            class="mt-2 max-w-[6rem] text-center font-label text-xs font-medium transition hover:opacity-90 sm:text-sm"
            :class="passoAtual >= passo.id
