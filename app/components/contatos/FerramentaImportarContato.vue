...
import MapeamentoDeColunas from '~/components/contatos/MapeamentoDeColunas.vue'
...
const importacaoConcluida = computed(
  () => !importacaoEmAndamento.value && importacaoTotal.value > 0 && importacaoProcessadas.value >= importacaoTotal.value && !importacaoErro.value,
)
...
async function executarImportacao(
  ...
  try {
    ...
    contatosStore.cacheLista = {}
    contatosStore.cacheBusca = {}
    await contatosStore.fetchPagina(workspaceId, { force: true })
    emit('importado')
    return true
  } catch (err) {
...
<template>
  ...
  >
    <template #extra>
      ...
      >
        Enviando em lotes de {{ TAMANHO_LOTE_IMPORTACAO }} registros. Conversas novas são criadas; existentes (mesmo canal + telefone) são atualizadas. Campos personalizados e status do funil mapeados são gravados após cada conversa.
      </p>
      <p
