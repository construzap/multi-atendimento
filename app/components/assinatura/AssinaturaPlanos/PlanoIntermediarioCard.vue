...
const PRECO_MENSAL = toNumber(config.public.planoIntermediario, 399)
const PRECO_POR_CANAL = toNumber(config.public.planoIntermediarioPorCanal, 50)
const LIMITE_PRODUTOS = 500
const MIN_CANAIS = 1
...
<template>
  <div
    class="flex h-full flex-col overflow-hidden rounded-2xl border border-warning/40 bg-surface-container-lowest shadow-sm dark:border-dark-warning/40 dark:bg-dark-surface-container-low"
  >
    <div class="flex items-center gap-4 px-5 py-5">
      <div
        class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl border border-warning/30 bg-warning-container/50 text-warning-on-container dark:border-dark-warning/30 dark:bg-dark-warning-container/40 dark:text-dark-on-warning-container"
        aria-hidden="true"
      >
    ...
    <div class="flex-1 border-t border-outline/30 px-5 py-4 dark:border-dark-outline/30">
      ...
      <p class="mt-2 font-body text-sm text-on-surface-variant dark:text-dark-on-surface-variant">
        Valor mensal + canais. Limite de até {{ LIMITE_PRODUTOS }} produtos.
      </p>
    </div>

    <div class="border-t border-outline/30 px-5 pb-5 pt-4 dark:border-dark-outline/30">
      <BaseButton size="sm" type="button" @click="abrirModal">
        Ver plano
      </BaseButton>
    </div>
  </div>
    ...
    <div class="space-y-6">
      ...
      <dl class="space-y-3 rounded-xl border border-outline/30 p-4 dark:border-dark-outline/30">
        ...
        >
          ...
          <dd class="font-headline font-bold tabular-nums text-warning dark:text-dark-warning">
            {{ valorTotalFormatado }}
          </dd>
        </div>
      </dl>
