...
type AtualizarBody = Partial<CriarCobrancaBody> & {
  id?: unknown
...
/**
 * POST /api/cobranca/atualizarcobranca
 * Body: mesmos campos de criar + `id` da cobrança.
 */
export default defineEventHandler(async (event): Promise<CriarCobrancaResponse> => {
  ...
  const body = (await readBody<AtualizarBody>(event).catch(() => null)) ?? {}
  ...
  if (!cobrancaRow) {
    throw createError({ statusCode: 500, statusMessage: 'Não foi possível atualizar a cobrança.' })
  }
  ...
  const cobranca = mapCobranca(cobrancaRow as Record<string, unknown>)
  const produtosMapped = ((produtosRows ?? []) as Record<string, unknown>[]).map(mapProduto)
  cobranca.produtos = produtosMapped
