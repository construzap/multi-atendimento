...
const CODIGO_COLUNA_ORIGEM_LEADS_NAO_CONFIGURADA = 'COLUNA_ORIGEM_LEADS_NAO_CONFIGURADA'
...
function trimRequired(raw: unknown, label: string): string {
  const t = String(raw ?? '').trim()
  if (!t) {
    throw createError({ statusCode: 400, statusMessage: `Informe o ${label}.` })
  }
  return t
}

function normalizeAndValidatePhone(input: unknown): string {
...
async function resolverColunaOrigemEFunil(
  ...
  // Resolve funil pela própria coluna (workspace pode ter vários funis).
  ...
  const funilId = toInt((coluna as { funil_id?: unknown } | null)?.funil_id)
  if (!coluna || funilId == null) {
    throw erroColunaOrigemNaoConfigurada()
  }
...
/**
 * POST /api/cobranca/criaconversa
 * Body: `{ workspace_id, id_canal, phone, name }`
 *
 * Insere conversa em `conversas` com `key` UUID.
 */
export default defineEventHandler(async (event): Promise<Conversa> => {
  ...
  const name = trimRequired(body?.name, 'nome')
  ...
  const canalWorkspaceId = toInt((canalRow as { workspace_id?: unknown } | null)?.workspace_id)
  if (canalWorkspaceId == null || canalWorkspaceId !== workspaceId) {
    throw createError({
      statusCode: 400,
      statusMessage: 'Canal não pertence a este workspace.',
    })
  }
  ...
  if (existing) {
    throw createError({
      statusCode: 409,
      statusMessage: 'Já existe conversa com este telefone neste canal.',
    })
  }
