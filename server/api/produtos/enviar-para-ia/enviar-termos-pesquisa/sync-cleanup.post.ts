...
import {
  ...
  deleteTermoDocumentById,
  listTermoVectorDocumentsChunk,
} from '../../../../utils/enviarParaIa/termosPesquisa/documentsTermosVectorStore'
import { fetchActiveTermoIdKeys } from '../../../../utils/enviarParaIa/termosPesquisa/termosIndexaveis'
...
/** POST /api/produtos/enviar-para-ia/enviar-termos-pesquisa/sync-cleanup — remove termos órfãos da vector store. */
export default defineEventHandler(async (event): Promise<SyncCleanupChunkResult> => {
  ...
  const [total, activeTermoIds, docs] = await Promise.all([
    countTermosByWorkspace(event, workspaceId),
    fetchActiveTermoIdKeys(event, workspaceId),
    listTermoVectorDocumentsChunk(event, workspaceId, offset, limit),
  ])
  ...
  for (const doc of docs) {
    const isOrphan = doc.termoId == null || !activeTermoIds.has(doc.termoId)
    if (!isOrphan) continue

    try {
      await deleteTermoDocumentById(event, doc.id)
      removed++
    } catch (err) {
