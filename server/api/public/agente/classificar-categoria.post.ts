...
import {
  executeStockTool,
  stockToolDefinition,
} from '../../../utils/agente/estoqueAgents/stock'
...
/**
 * POST /api/public/agente/classificar-categoria
 *
 * Body: {
...
export default defineEventHandler(async (event): Promise<AgenteEspecializadoResponse> => {
  ...
  const termoSemNumeros = produtos.replace(/[0-9]/g, '').trim() || produtos
  const userMessage = `produto solicitado: ${termoSemNumeros}`
  ...
  const result = await runSpecializedAgentLoop(event, {
    ...
    executeTool: (name, rawArgs) => {
      ...
      return executeStockTool(event, rawArgs, workspace_id)
    },
    openai: {
