-- View de listagem: termos via produto_termo_de_pesquisa_vinculo (N:N).
-- Pré-requisitos: produto_termo_de_pesquisa, produto_termo_de_pesquisa_vinculo.
      ...
      select
        json_agg(
          to_jsonb(var.*) || jsonb_build_object(
            ...
            coalesce(
              (
                select json_agg(json_build_object('id', t_v.id, 'nome', t_v.nome) order by t_v.nome)
            ...
            (
              select string_agg(t_v.nome, ' ' order by t_v.nome)
              from public.produto_termo_de_pesquisa_vinculo v_v
              join public.produto_termo_de_pesquisa t_v on t_v.id = v_v.termo_id
              where v_v.produto_id = var.id
            )
...
comment on view public.view_produtos_com_variacoes is
  'Produtos pai com variações, imagens, categoria e termos (produto_termo_de_pesquisa_vinculo).';
